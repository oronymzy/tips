...
| list all commits in which a fixed string was added or removed                 | `git log --all -p --reverse --source -S 'foobar'`      | {==foobar==} represents the fixed string to be searched for.
...
| search through commit messages                                                | `git log -i --grep="foobar"`                           | {==foobar==} represents the pattern to search for.                                                                                                | Regular expressions are supported. The `-i` [option](https://git-scm.com/docs/git-log#Documentation/git-log.txt--i) enables case insensitivity for the `--grep` search.

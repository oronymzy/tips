...
: In [C++](https://en.wikipedia.org/wiki/C++), the [logical inclusive OR](https://en.wiktionary.org/wiki/inclusive_or) operator connects the logic of multiple expressions.[^logicaloperatordef-note4] The operator is two pipe characters (`||`), for example `a || b` means `a` **or** `b`.[^logicaloperatordef-note3]
